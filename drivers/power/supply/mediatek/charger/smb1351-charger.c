...
#define USBIN_COLLAPSE_TIME_MS ((unsigned int)1200)
						...
						u8 status)
{
	...
	if (status == 0) {
		...
		sub_time = jiffies_to_msecs(s_vbusin_uv_time - f_vbusin_uv_time);
		if (sub_time < USBIN_COLLAPSE_TIME_MS) {
			_smb1351_enable_hvdcp_det(chip, false);
			pr_err("f_time:%lu; s_time:%lu; sub:%lu sub_time:%dms\n", f_vbusin_uv_time, s_vbusin_uv_time, (s_vbusin_uv_time - f_vbusin_uv_time), sub_time);
		}
	}
